...
def write_predictions_to_file(model_predictions, headline_source, file_headlines):
    ...
    existing_file_path_check = "headline_scores_"+headline_source+\
        "_"+str(datetime.date.today().year)+"_"+\
            str(datetime.date.today().month)+"_"+\
                str(datetime.date.today().day)+".txt"
    if os.path.exists("./"+existing_file_path_check):
        with open("./"+existing_file_path_check, 'a+', encoding='utf-8') as file:
            for prediction_index, prediction_value in enumerate(model_predictions, start=0):
                file.write(str(prediction_value)+\
                    ","+str(file_headlines[prediction_index]))
    else:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Error: Running this program requires providing two arguments, \
